...
exports.run = async (bot, message, args) => {
    ...
    message.channel.send({embed:{ color: Math.floor(Math.random() * (0xFFFFFF + 1)), description:(`Doğruluk mu yoksa cesaret mi?`)}})
    let uwu = false;
    while (!uwu) {
        ...
        if (choice !== 'cesaret' && choice !== 'c') {
            message.channel.send({embed:{ color: Math.floor(Math.random() * (0xFFFFFF + 1)), description:(`Lütfen sadece **doğruluk (d)** veya **cesaret (c)** ile cevap verin.`)}})
        }
